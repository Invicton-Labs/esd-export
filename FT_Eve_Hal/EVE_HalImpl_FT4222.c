...
#define FT4222_WRITE_HEADER_SIZE (EVE_CHIPID >= EVE_BT820 ? 4 : 3)
...
static inline bool rdBuffer(EVE_HalContext *phost, uint8_t *buffer, uint32_t size)
{
	...
	while (size)
	{
		...
		const uint32_t readSizeMax =
			FT4222_TRANSFER_SIZE_MAX;

		if (size <= readSizeMax)
			bytesPerRead = size;
		else
			bytesPerRead = readSizeMax;
		...
		if (phost->SpiChannels == EVE_SPI_SINGLE_CHANNEL)
		{
			...
			const uint16_t writeSize = EVE_CHIPID >= EVE_BT820 ? 4 : (3 + phost->SpiDummyBytes); /* 3 address and chosen dummy bytes */
			status = FT4222_SPIMaster_SingleWrite(phost->SpiHandle, hrdpkt, writeSize, &sizeWritten, FALSE);

			if ((status != FT4222_OK) || (sizeWritten != writeSize))
			{
...
void EVE_Hal_hostCommand5(EVE_HalContext *phost, EVE_82X_SPI_COMMANDS_T cmd, uint8_t param0, uint8_t param1, uint8_t param2)
{
	...
	uint8_t transferArray[5] = { prefix, cmd, param0, param1, param2 };
	eve_assert(phost->Status == EVE_STATUS_OPENED);

	flush(phost);
	eve_printf_debug("SPI cmd: 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", prefix, cmd, param0, param1, param2);
...
bool EVE_Hal_powerCycle(EVE_HalContext *phost, bool up)
{
	...
	if (up)
	{
		...
		EVE_sleep(150); /* Must wait 150µs before calling ACTIVE */
	}
	else
	{
		FT4222_STATUS status = FT4222_OTHER_ERROR;

#if 0 /* Don't need to cycle up to shutdown */
		if (FT4222_OK != (status = FT4222_GPIO_Write(phost->GpioHandle, phost->PowerDownPin, 1)))
		{
			eve_printf_debug("FT4222_GPIO_Write error = %d\n", status);
			res = false;
...
void EVE_Hal_setSPI(EVE_HalContext *phost, EVE_SPI_CHANNELS_T numchnls, uint8_t numdummy)
{
	...
	if (EVE_CHIPID >= EVE_BT820)
	{
		...
		/* SPI_WIDTH is 2 bits at bit 8 and 9, same enum as previous gen */
		syscfg = EVE_Hal_rd32(phost, SYS_CFG);
		syscfg = (numchnls << 8) | (syscfg & ~(3UL << 8));
		EVE_Hal_wr32(phost, SYS_CFG, syscfg);

		/* Switch FT4222 to multi channel SPI mode */
		setSPI(phost, numchnls, 0); /* numdummy is not used on BT820 */
	}
	else
#endif
	{
